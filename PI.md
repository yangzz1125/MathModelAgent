...
For contract-v3, the Host puts a stage-specific workspace context list into every prompt. It guides relevance and reduces unnecessary model search; it is not a filesystem confidentiality or path-access boundary. Inventory is directed to copied inputs; later Method, Spike, Execution, and read-only Review stages are directed to the current problem's declared inputs, accepted dependency artifacts, and current Method/Spike or candidate lineage as applicable. Agents must not search Host implementation, tests, other workspaces, repository history, or unlisted superseded planning versions. Method Planning reads the figure catalog to choose a reference; Execution and Scientific Review receive the selected catalog entries and previews. Independent reads should be batched in one turn. Enforced integrity controls remain Reviewer tool-type restriction, Windows Host locks, deterministic stage write-scope validation, and frozen hashes.

After all problems are scientifically accepted, the Host gives Sol a paper context list assembled from the problem statement, execution plan, completeness receipt, accepted reviews, and frozen artifacts. Paper Planning must not search `pi/`, tests, other workspaces, repository history, or superseded Method/Spike versions. Sol creates `paper_plan.json`, mapping every accepted claim to equations, algorithms, result evidence, independent validation, robustness, a non-empty applicability limitation, figures, and citation needs. Initial and repair prompts share a typed coverage example; `model_or_equations` and `limitations` are non-empty string lists, and the Host reports sibling type errors together. A separately labeled eligible evidence set excludes audits/receipts and other problems; freezing a file does not make it scientific proof. Required scientific figures are the exact accepted vector masters, not optional previews or paper-local replacements. Page range is advisory; missing scientific content is a hard failure. Luna receives the same context plus the paper plan and current Diagram artifacts, reads the writing skill for template rules, and uses shell commands only to compile/render/validate the paper. It writes the paper and `paper/paper_manifest.json`; manifest anchors must be unique, non-overlapping, and include a truthful limitation anchor for every claim. The Host also rejects explicit LaTeX bibliography entries that are unused or unresolved, duplicate contents sequences, consecutive forced page breaks, and short reference lists forced onto a separate page. Final Sol Document Verification receives an explicit list of paper source, manifest, existing log/PDF, rendered pages, and accepted evidence; it is capability-restricted to `read/grep/find/ls` and returns strict JSON without compiling, rendering, or writing files. For contract-v3, the Host uses `pdfinfo` for the actual physical page count and `pdftoppm` at 160 DPI to regenerate every color/grayscale page after Writing passes its gates. This work is cancellable and bounded; producer logs and previews are not authoritative. PDF and page hashes are persisted in Host state before the read-only Reviewer snapshot, then rechecked at final acceptance. The Host also checks literal figure inclusions in reachable LaTeX/Typst source against the scientific manifest; conceptual diagrams may remain outside problem-owned figure directories. Dynamic TeX programming and scientific visual meaning still require Reviewer inspection. Contract-v1/v2 retain their existing acceptance paths; the new frozen-figure and Host-render contracts apply to v3. The Host rechecks the frozen chain and PDF, writes `reports/VERIFY_REPORT.md`, and alone marks completion. A valid rejection can route through at most two bounded Luna paper repairs.
