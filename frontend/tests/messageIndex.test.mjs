...
test('history does not overwrite newer live updates',()=>{
  const rows=mergeHistory([{id:'a',content:'cached'}],[{id:'a',content:'old'},{id:'b',content:'history'}],[{id:'a',content:'new'}]);
  assert.equal(rows.find(x=>x.id==='a').content,'new');assert.equal(rows.length,2);
});
test('revisited history replaces old cached fragments but retains unsnapshotted ids',()=>{
  const rows=mergeHistory([{id:'a',content:'partial'},{id:'local',content:'pending'}],[{id:'a',content:'final'}]);
  assert.equal(rows.find(x=>x.id==='a').content,'final');
  assert.equal(rows.find(x=>x.id==='local').content,'pending');
});
