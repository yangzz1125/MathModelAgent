...
Scientific acceptance happens per problem before artifacts are frozen. Paper Planning and Writing receive a Host-assembled allowlist of the problem statement, execution plan, completeness receipt, accepted reviews, frozen artifacts, paper plan, and current Diagram outputs as applicable. Do not inspect `pi/`, tests, other workspaces, repository history, or superseded Method/Spike versions to rediscover the contract; use shell commands during Writing only for paper compilation, rendering, and validation. Missing evidence must be reported rather than recovered from an unlisted path. Document verification later checks content coverage, numerical consistency, compilation, references, and PDF quality; it does not retroactively replace scientific review. The Document Reviewer remains read-only and returns strict JSON; the Host writes `reports/VERIFY_REPORT.md` and decides completion. One malformed JSON response receives a read-only protocol retry without triggering a paper repair. New contract-v3 projects do not wait for manual repair: stay within bounded automatic repair/replan instructions, or fail clearly when evidence or indispensable input cannot be recovered. Historical contract-v1/v2 workspaces keep their original state machine and are never migrated in place.
