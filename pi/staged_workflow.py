...
def validate_method_card(
    ...
    invalid_outputs = [
        ...
        if not output.startswith(allowed_outputs[:3]) and output != allowed_outputs[3]
    ]
    if invalid_outputs:
        raise ContractError(
            f"{problem_id} output is outside its artifact boundary: {invalid_outputs}; "
            f"allowed: {list(allowed_outputs)}; paper/ belongs to downstream Writing"
        )
...
{problem_shape}
```
All `inputs` and `outputs` entries are literal relative path strings, never descriptions such as "candidate metrics". `outputs` must include at least one path under each of `code/{problem_id}/`, `results/{problem_id}/`, and the exact file `reports/{problem_id}_RESULTS.md`; figure artifacts use `figures/{problem_id}/`. Every artifact generated by this problem's Execution stage must be declared in `outputs`. A requested final paper is a downstream Writing deliverable, not an Execution output: keep its requested-output claim and paper-readiness evidence, but never declare paper/ paths here. Validate the scientific evidence that Writing needs; do not promise to generate or independently validate the final PDF during this problem.

Claim `type` is exactly one of `identity_or_constraint`, `feasibility`, `optimality`, `event_or_boundary`, `approximation`, `estimate_or_prediction`, `ranking_or_decision`, or `other`. Every claim adds evidence_level, requested_output_ids, and limitations (non-empty for Level B/C). Every requested-output ID needs at least one A_certified or B_bounded_numerical claim; C_exploratory is supplementary only. The union of all `independent_validation[].claims` must equal the complete set of claim IDs; every claim requires independent validation, including figure, limitation, implementation-constraint, and paper-readiness claims. ID coverage alone is insufficient: each independent-validation method must substantively test the full quantifier, domain, regimes, boundary sides, outputs, and evidence obligations stated by every mapped claim, not one representative point for a regime-wide claim. Every failure_semantics object has exactly id, event_id, category, condition, and action; category is `domain_event`, `mathematical_infeasibility`, `numerical_failure`, `data_or_input_failure`, or `decision_outcome`. Conditions describing the same physical event share event_id and category.
...
The top-level `finite_domain`, `witness_strategy`, and `gap_or_tail_exclusion` values are each one non-empty string, not objects or arrays. State finite domains, witnesses/brackets, gap/tail exclusion, mutually exclusive failure semantics, independent validation, figure specs, and a realistic runtime limit. cost_model has exactly operation, estimated_operations, estimated_seconds, memory_mb, scaling. spike_spec has exactly questions, representative_cases, required_metrics and required_witnesses; each list entry is {{id,description}}, IDs are globally unique, and only required_witnesses may be empty. Scope the main Spike to the smallest uncertainties that one command can completely answer inside its computed budget. Do not require a production figure, vector/PDF export, full grayscale suite, or final annotation/layout audit in the main Spike; those belong to Execution and pixel-level Scientific Review. A figure Spike may check dependency/font availability and one lightweight Agg canvas. If one cold-start uncertainty genuinely needs more time, define exactly one targeted question that a Reviewer may authorize for the single supplemental Spike. Prefer the cheapest scientifically honest evidence; never invent an unaffordable formal certificate. Stop after the two method artifacts."""
...
Read the immediately preceding immutable proposal before writing this version. Preserve all claim IDs. The Host has exhausted ordinary audits and authorized only these listed evidence changes:
{evidence}
...
```
The exact coverage below is authoritative, including empty arrays, and overrides the illustrative IDs in the skeleton. Map answered_question_ids to questions, probe_scope to cases, benchmarks[].metric_id to benchmark_metric_ids, and witnesses[].witness_id to witness_ids; never put question IDs in probe_scope. Each benchmark has exactly metric_id, name, operations, seconds, throughput, unit; operations, seconds, and throughput are positive finite numbers and `throughput = operations / seconds` within 5% (use the measured operation count, not `1 / seconds`). Each witness has exactly witness_id, type, summary, artifact_paths.{target_note}

All bash calls and repairs share the one declared budget. Main and supplemental Spike share at most two local repairs in total; a supplemental run does not reset that allowance. No additional computation is permitted once the time budget is exhausted. Use at most one bounded probe command, run from the workspace root. In probe.py resolve workspace files from `Path.cwd()`; do not infer the root with brittle `Path(__file__).parents[N]` indexing (supplemental has a different depth). Before any optional slow operation, persist the measurements already obtained and explicit unresolved risks; update them atomically after successful measurements. An incomplete checkpoint is not an acceptable candidate: never fabricate answered IDs, positive benchmarks, or witnesses to pass the schema. A planning figure probe must not invoke TeX, LaTeX, a PDF compiler, package installation, or network access. If applying a Matplotlib/SciencePlots style, set `text.usetex=False` after the style; use only a lightweight Agg canvas or PNG render and record a bounded timeout honestly. Every artifact, including probe.py and witness files, must be listed and stay inside this Spike directory. Timeout or process failure is numerical/planning feasibility evidence, never mathematical infeasibility or a domain event. Stop after writing the candidate report."""
...
def spike_repair_prompt(
    ...
    *,
    ...
    base = spike_prompt(card, supplemental=supplemental, supplemental_ids=supplemental_ids)
    details = "\n".join(f"- {error}" for error in errors)
    return f"""{base}

The Host rejected the current Spike artifacts. This is a same-version local repair under the same immutable Method Card, not a new Spike. All bash calls and repairs share the original budget. Reuse valid existing computation and rerun only the smallest missing check within the remaining budget. Re-read the current probe.py, spike_report.json and witness files. Correct every gate error without changing measured values dishonestly. Do not create a new Method Card or modify input, accepted dependencies, code, results, figures, reports, or execution_plan.json. Stop after repairing this Spike directory.
...
Do not trust candidate status, planner claims, worker prose, or self-reported visual-check names. Do not edit/create files or run expensive computations. Use the `read` tool on every produced PNG preview so the actual image is attached for pixel-level inspection; do not infer visual quality from JSON, source code, or `labels_checked` claims. Reject any annotation, legend, tick label, or axis title that crosses an axes/figure boundary, overlaps another element, is unreadable at final width, or contradicts reported provenance.
...
def paper_planning_prompt(
    ...
    eligible_evidence: dict[str, list[str]] | None = None,
) -> str:
    scope = _evidence_scope(evidence_paths)
    shape = json.dumps({"schema_version": 1, "plan_version": plan_version,
                       "recommended_page_range": [8, 10], "coverage": [PAPER_COVERAGE_EXAMPLE]},
                      ensure_ascii=False, indent=2)
...
Use this complete typed JSON shape; preserve field types, replace illustrative values, and repeat coverage once per accepted claim. Non-empty example arrays must remain non-empty lists of strings; empty example arrays may remain empty. In particular model_or_equations and limitations are lists, never strings.
```json
{shape}
```
Eligible frozen scientific evidence by problem (distinct from the general context list):
{json.dumps(eligible_evidence or {}, ensure_ascii=False, indent=2)}

Write paper_plan.json and reports/PAPER_PLAN.md. paper_plan.json uses schema_version=1, the active plan_version, an advisory recommended_page_range [min,max], and coverage with exactly one entry per accepted claim. Each coverage entry requires claim_id, problem_id, section_id, interpretation_and_assumptions, non-empty model_or_equations, algorithm_and_stopping, non-empty frozen result_evidence and validation_evidence paths, sensitivity_or_robustness, approximation_ids, non-empty limitations, figures, and citations_needed. `figures` must equal the accepted Method figure_specs vector_path set mapped to that claim, not PNG previews or invented replacements. `result_evidence` and `validation_evidence` may contain only accepted current-problem artifact paths present in the Host-frozen evidence list. Never use `reports/PLAN_COMPLETENESS.json`, inventory/method/audit files, execution_plan.json, paper_plan.json, or general reports as claim evidence; Host receipts and plans describe coverage but do not prove a scientific claim. Every claim must state its applicability boundary or limitation; every declared approximation must also be disclosed there. Page range is advisory, but choose it proportionally to problem complexity and avoid duplicate sections or padding. Scientific content coverage is mandatory. Stop before writing the paper."""


def paper_plan_repair_prompt(
    errors: list[str], *, plan_version: int = 1,
    evidence_paths: list[str] | None = None,
    eligible_evidence: dict[str, list[str]] | None = None,
) -> str:
    details = "\n".join(f"- {error}" for error in errors)
    base = paper_planning_prompt(plan_version, evidence_paths, eligible_evidence)
    return f"""{base}

The deterministic paper-plan coverage gate rejected paper_plan.json. Repair only paper_plan.json and reports/PAPER_PLAN.md, cover every accepted claim and approximation using only accepted current-problem paths from the Host-frozen evidence list, and stop. Do not use `reports/PLAN_COMPLETENESS.json`, inventory/method/audit files, plans, or general reports as `result_evidence`/`validation_evidence`; those are context or receipts, not scientific evidence. Do not write the paper or invent experiments.

Gate evidence:
{details}"""


PAPER_LAYOUT_CONTRACT = r"""Paper layout contract (overrides the skill's [H] examples):
For Chinese CUMCM LaTeX, read project.json workflow.paper_layout and copy the pinned $MATHMODELAGENT_ROOT/pi/latex/<paper_layout>.tex unchanged to paper/cumcm-layout.tex. If the project has no pinned layout, use cumcm-v2.tex. Never upgrade a pinned historical layout implicitly. Use \documentclass[a4paper,12pt]{ctexart}, then \input{cumcm-layout.tex} in the preamble. Replace the old template preamble and title/contents helper definitions with this include; do not redefine papertitle or papercontents. This shared layout loads geometry, amsmath, amssymb, graphicx and booktabs; do not reload geometry or override its margins, font size or line spacing to hit a page target. Keep normal supplementary packages and actual manuscript content in main.tex/sections.
Use \papertitle{the actual title}, the abstract and keywords, then exactly one \papercontents before the first body section. In the current layout the abstract uses the body font size and full text width, with only a two-Chinese-character first-line indent; do not wrap it in quotation/quote or add manual spaces. Do not use default \maketitle with empty author/date rows or repeat \tableofcontents. The intentional abstract/contents/body boundaries are not padding; do not force each body section or a short bibliography onto a new page. Other competitions/languages/Typst keep their matching templates rather than this CUMCM layout.
For LaTeX figures and short tables prefer [htbp] instead of blanket [H]. Allow nearby text to fill space when a figure does not fit; inspect reading order so floats remain near their first reference. Do not shrink frozen figures below their declared readable size or redraw them. Use longtable for genuinely multipage tables. Do not patch gaps with negative vspace, vfill, smaller fonts or removed scientific content. Inspect the title-to-abstract gap, contents boundaries and every page before/after a large figure/table. Normal final-page/front-matter whitespace is acceptable; unexplained internal gaps and large avoidable body-page holes need repair. Page count is advisory, not a reason to pad or compress."""
...
The manifest scientific figures must exactly match the same claim in paper_plan.json. Embed only the accepted frozen vector master using a literal includegraphics/image path. Never replace or replot a scientific figure under paper/. A frozen figure defect is not a paper-layout repair. Separate non-scientific diagrams must remain under figures/ outside problem-owned directories. The Host regenerates authoritative color/grayscale physical-page images from the PDF before Verify; producer previews are advisory only.
...
The manifest `figures` list for each claim must exactly equal that claim's `figures` list in paper_plan.json. Every listed scientific figure must be included literally by the reachable paper source; unplanned replacements under paper/ are forbidden.
...
This current Host contract overrides any skill instruction to run commands or write a report: do not edit/create files, run compilation, render pages, or use shell commands. Use `read` on every listed `paper/rendered_pages/page-NN.png` and `page-NN-gray.png` image so visual judgments come from the actual persisted page pixels, not manuscript claims or log text.

Confirm every accepted claim is substantively covered; all six manifest anchors per claim are unique and truthful; frozen evidence and paper numbers agree; required models, results, independent validations, sensitivity boundaries, conclusions, and limitations are present; every listed reference is cited and every citation resolves; the existing log/PDF establish successful compilation; and every rendered page is readable. For every figure, verify accepted provenance, paper-language labels, units, scales, legends, grayscale distinction, and absence of overlap or clipping. Confirm exactly one contents sequence and no unnecessary forced separation before a short reference list. Inspect title-to-abstract spacing, contents/body boundaries, and body pages before and after each figure/table for large avoidable holes or stretched gaps. Cite the physical page and offending element in any requested layout repair; normal front-matter and final-page whitespace is not itself a failure. Short page count alone is a warning, not a hard failure.
