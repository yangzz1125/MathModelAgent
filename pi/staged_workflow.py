...
Re-read the current probe.py, spike_report.json, witness files, and planning/methods/{problem_id}/v{version}/method_card.json. spike_report.json must have exactly these top-level keys and no others: schema_version, status, problem_id, method_spec_sha256, budget_seconds, actual_runtime_seconds, answered_question_ids, probe_scope, benchmarks, estimated_full_runtime_seconds, peak_memory_mb, witnesses, unresolved_risks, artifact_paths. Correct the rejected schema, coverage, artifact declaration, or bounded probe evidence without changing measured values dishonestly. All bash calls and repairs share the original budget: run at most one bounded probe command from workspace root, resolve workspace files with `Path.cwd()` rather than `Path(__file__).parents[N]`, never use TeX/LaTeX/PDF compilation, package installation, or network access, and force `text.usetex=False` after any plotting style. Write a structurally complete truthful checkpoint before optional slow work so another timeout does not remove the report. Reuse valid existing computation and rerun only the smallest missing check within the remaining budget. {target_note} A timeout or failed probe remains numerical/planning feasibility evidence, never mathematical infeasibility. Stop after the repaired Spike artifacts exist."""
...
Fix every item under the report's hard-error and required-repairs sections, including truthful manifest anchors. Preserve accepted numbers and embed only the accepted frozen vector master; never create a repaired/replotted figure under paper/ or substitute it for frozen provenance. For each paper-layout defect, edit the paper source, compile twice, rebuild `paper/rendered_pages/page-NN.png` and `page-NN-gray.png` for every physical page at at least 160 DPI, inspect the exact failed page images, and iterate within this repair turn until all cited labels, axes, legends, tables, and margins are visibly separated and unclipped. If the defect exists inside a frozen accepted figure rather than paper layout, report that it cannot be repaired in paper/; do not fabricate replacement evidence. Use the paper language for chart prose. Confirm all manifest anchors are unique, semantically correct literal substrings before stopping. Confirm every explicit bibliography item is cited in the body and every citation resolves. Remove duplicate contents pages, consecutive forced page breaks, and forced separation before a short reference list. Use TEST as a neutral control number only when the problem supplied none. Do not run verification or write VERIFY_REPORT.md yourself."""
...
Every chart's prose labels, axis titles, legends, and annotations must use {language}; mathematical symbols may remain standard. Embed the accepted frozen vector master declared by its figure provenance. Do not rebuild a chart in paper/. Keep labels visibly separated from ticks, curves, axes boundaries, and each other. Compile twice. Create `paper/rendered_pages/`, render every physical PDF page at at least 160 DPI as `page-NN.png`, and create a grayscale copy of every page as `page-NN-gray.png`; NN is two digits starting at 01. Inspect these persisted images for clipping, overlap, blank pages, figure provenance fidelity, grayscale readability, and incoherent whitespace before stopping. The Host requires one color and one grayscale image for every page reported by `paper/main.log`.
